...
char* text;
SDL_Rect rect;
char* message; // добавлено поле message
} MenuItem;
...
MenuItem menuItems[] = { // изменен тип с struct MenuItem на MenuItem
{"Пункт 1", {10, 10, 100, 50}, "Сообщение 1"}, // добавлено поле message
{"Пункт 2", {10, 70, 100, 50}, "Сообщение 2"}, // добавлено поле message
{"Пункт 3", {10, 130, 100, 50}, "Сообщение 3"} // добавлено поле message
};
...
// Сохранение текста выбранного пункта меню
if (selectedItemText != NULL) {
free(selectedItemText);
}
selectedItemText = strdup(menuItems[selectedItem].message); // изменено на поле message
printf("%s\n", selectedItemText);
}
}
}
